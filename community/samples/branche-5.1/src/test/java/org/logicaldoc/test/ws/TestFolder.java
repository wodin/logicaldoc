...
public class TestFolder extends TestCase {
	...
	public void testFolderCRUD() throws IOException {
		...
		/**
		 * public String createFolder(String sessionId, String
		...
		try {
			...
			String result = client.createFolder(sessionId, "myFirstFolder", parentFolderId);
			System.out.println(result);
			assertFalse("error".equals(result));

			// Folder Retrieval
			long createdFolder = Long.parseLong(result);
			FolderContent fc = client.downloadFolderContent(sessionId, createdFolder);
			...
			client.renameFolder(sessionId, createdFolder, "New Folder Name");
			fc = client.downloadFolderContent(sessionId, createdFolder);
			System.out.println(fc.getName());

			// Folder Delete
			result = client.deleteFolder(sessionId, createdFolder);
