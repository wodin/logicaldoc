These examples have been created and tested with:
 Windows Vista Business Ed. SP1, PHP 5.2.8 and LogicalDOC 4.0.2

1) Preliminary operations:
   a) ensure that you have the soap extension enabled (to verify try to launch from the shcell: php SOAP_printGoogleSearchWsdl.php)
      uncomment the property: extension=php_soap.dll in your php.ini
      
   b) set to false the mtom-enabled property of the LogicalDOC 4.0.2 webservice plugin.
...
Note: the Default Folder for the downloads is C:\tmp anyway you can change this in the scripts.

Final Note: the scripts refer to a LogicalDOC 4.0 Webservice running on port 8080 and accessible with
the path http://localhost:8080/logicaldoc/services/Dms

if your logicaldoc runs on a different port or with a different path (this is the case of a LD running as ROOT webapp)
