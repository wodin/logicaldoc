...
public class TermSelector {
	...
	/**
	 * Selects n terms the mostly occurred in all document being in a given
	 * search result.
	 * 
	 * @param searchResult
	 ...
	 * @return
	 */
	public Collection<TermEntry> getTopTerms(Collection<Result> searchResult, int count) {
		Iterator<Result> iter = searchResult.iterator();
		TermDAO termDao = (TermDAO) Context.getInstance().getBean(TermDAO.class);

		while (iter.hasNext()) {
			Result rs = iter.next();
			Collection<Term> termcoll = termDao.findByDocId(rs.getDocId());
			Iterator<Term> termiter = termcoll.iterator();
			...
			while (termiter.hasNext()) {
				...
				if (terms.containsKey(term.getStem())) {
					...
					if (term.getOriginWord().length() < entry.getOriginWord().length()) {
	...
	protected TermEntry getTopWord() {
		TermEntry entry = new TermEntry();
		Enumeration<String> enum1 = terms.keys();
		String topterm = "";
		double topvalue = 0d;

		while (enum1.hasMoreElements()) {
			String term = enum1.nextElement();
