...
public class SetPassword extends Window {
	...
	public SetPassword(final long userId) {
		...
		sizeValidator.setErrorMessage(I18N.getMessage("errorfieldminlenght", Integer.toString(Session.get().getUser()
							...
							new AsyncCallback<Integer>() {
								...
								public void onSuccess(Integer ret) {
									if (ret.intValue() > 0) {
										...
										// opened
										if (ret == 1)
											Log.warn(I18N.getMessage("wrongpassword"), null);
										else if (ret == 2)
											Log.warn(I18N.getMessage("passwdnotnotified"), null);
										else
											Log.warn(I18N.getMessage("genericerror"), null);
									}
