...
public class ExportArchivesPanel extends VLayout {
	...
	public void init() {
		...
		ListGridField creator = new ListGridField("creator", I18N.message("creator"), 100);
		ListGridField closer = new ListGridField("closer", I18N.message("closedby"), 100);
		...
		list.setDataSource(new ArchivesDS(GUIArchive.MODE_EXPORT));
		...
		ToolStripButton incremental = new ToolStripButton();
		incremental.setTitle(I18N.message("incrementalarchives"));
		incremental.addClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				// Implement this
				SC.say("To be implemented");
			}
		});
		if (Feature.visible(Feature.INCREMENTAL_ARCHIVES)) {
			toolStrip.addButton(incremental);
			if (!Feature.enabled(Feature.INCREMENTAL_ARCHIVES)) {
				incremental.setDisabled(true);
				incremental.setTooltip(I18N.message("featuredisabled"));
			}
		}
		...
		list.addSelectionChangedHandler(new SelectionChangedHandler() {
			...
			public void onSelectionChanged(SelectionEvent event) {
				...
				showDetails(Long.parseLong(record.getAttributeAsString("id")),
						Integer.toString(GUIArchive.STATUS_OPENED).equals(record.getAttributeAsString("status")));
			}
		});
