 ...
 * @since 6.1
 */
public class BarcodesEnginePanel extends VLayout {
	...
	public BarcodesEnginePanel() {
		...
		service.getInfo(Session.get().getSid(), new AsyncCallback<GUIBarcodeEngine>() {
			...
			public void onSuccess(GUIBarcodeEngine engine) {
				BarcodesEnginePanel.this.engine = engine;

				Tab engineTab = fillEngineTab();

				Tab schemeTab = fillSchemeTab();

				BarcodesEnginePanel.this.processingQueueTab = fillProcessingQueueTab(100);

				BarcodesEnginePanel.this.tabSet.setTabs(engineTab, schemeTab, processingQueueTab);
	...
	private Tab fillEngineTab() {
		Tab searchEngineTab = new Tab(I18N.message("barcodeengine"));
		...
		save.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				if (vm.validate()) {
					BarcodesEnginePanel.this.engine.setIncludePatters((String) values.get("includePatters"));
					BarcodesEnginePanel.this.engine.setExcludePatters((String) values.get("excludePatters"));
					String btch = vm.getValueAsString("batch");
					if (btch == null || "".equals(btch.trim()))
						BarcodesEnginePanel.this.engine.setBatch(0);
					else
						BarcodesEnginePanel.this.engine.setBatch(new Integer(btch));

					String threshold = vm.getValueAsString("resolutionthreshold");
					if (btch == null || "".equals(btch.trim()))
						BarcodesEnginePanel.this.engine.setImageThreshold(0);
					else
		...
		searchEngineTab.setPane(engineTabPabel);
