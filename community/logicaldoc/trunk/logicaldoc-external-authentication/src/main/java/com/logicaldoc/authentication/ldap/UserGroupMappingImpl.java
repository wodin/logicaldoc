...
public class UserGroupMappingImpl implements UserGroupMapping{
	...
	public LdapUser getUserByUniqueAttributeIdentiferValue(String identifierValue){
		...
		for (String groupDN : userBases) {
			List<LdapUser> userList = this.ldapTemplate.search(
					groupDN,"(& (objectClass=" + ldapUserGroupContext.getUserClass() + ") (" + ldapUserGroupContext.getLogonAttribute() + "=" + identifierValue+"))", 
					sc, this.userMapper);
			if(userList.isEmpty() == false){
				LdapUser ldapUser = userList.get(0);
				ldapUser.dn = new TrimmedDistinguishedName(ldapUserGroupContext.getUserIdentiferAttribute() + "=" + ldapUser.rdn + ","+groupDN);
				return ldapUser;
			}
		}
	...
	public List<LdapGroup> getAllGroups() {
		...
		for (String groupDN : groupBases) {
			...
			List<LdapGroup> groupList = this.ldapTemplate.search(
					groupDN,"objectClass="+this.ldapUserGroupContext.getGroupClass(), sc, this.groupMapper);
			
			for(LdapGroup _ldapGroup : groupList){
				_ldapGroup.dn = new TrimmedDistinguishedName(this.ldapUserGroupContext.getGroupIdentiferAttribute() + "="+_ldapGroup.name + "," + groupDN);
	...
	public List<LdapUser> getAllUsers() {
		...
		for (String groupDN : groupBases) {
			...
			List<LdapUser> groupList = this.ldapTemplate.search(
					groupDN,"objectClass=" + ldapUserGroupContext.getUserClass(), sc, this.userMapper);
			
			//postprocessing user to retrieve users dn
			for(LdapUser _ldapUser : groupList)
				_ldapUser.dn = new TrimmedDistinguishedName(ldapUserGroupContext.getUserIdentiferAttribute() + "=" + _ldapUser.rdn + ","+groupDN);
