...
public class VersionHistoryResourceImpl extends DeltaVResourceImpl implements VersionHistoryResource {
			...
			DavResourceLocator loc = locator.getFactory()
					.createResourceLocator(
							...
							locator.getResourcePath() + "/vstore/"
									...
									+ resource.getName());
			versions.add(new VersionResourceImpl(loc, factory, session, config,
					resource));
		}
    ...
    protected void initProperties() {
		if (!propsInitialized) {
			...
			properties.add(new ResourceType(new int[] {
					ResourceType.COLLECTION, ResourceType.VERSION_HISTORY }));

			try {
				properties.add(new HrefProperty(
						VersionHistoryResource.ROOT_VERSION, "", false));
			} catch (Exception e) {
