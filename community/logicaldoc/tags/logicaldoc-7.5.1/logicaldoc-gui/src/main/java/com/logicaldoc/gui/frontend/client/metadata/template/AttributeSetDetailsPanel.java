...
public class AttributeSetDetailsPanel extends VLayout {
	...
	protected AttributeSetServiceAsync service = (AttributeSetServiceAsync) GWT.create(AttributeSetService.class);
		...
		closeImage.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				if (attributeSet.getId() != 0) {
					service.getAttributeSet(attributeSet.getId(), new AsyncCallback<GUIAttributeSet>() {
						@Override
	...
	protected void onSave() {
		if (validate()) {
			service.save(attributeSet, new AsyncCallback<GUIAttributeSet>() {
				@Override
				public void onFailure(Throwable caught) {
					Log.serverError(caught);
				}
