 ...
 * @since 6.0
 */
public class AccountFiltersPanel extends AccountDetailsTab {
	...
	public AccountFiltersPanel(GUIEmailAccount account, ChangedHandler changedHandler) {
		...
		addRule.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				list.addData(record);
				AccountFiltersPanel.this.changedHandler.onChanged(null);
			}
		});
	...
	private void refresh() {
		if (list != null && formsContainer.contains(list))
			formsContainer.removeChild(list);
		...
		list = new ListGrid() {
			...
			protected Canvas createRecordComponent(final ListGridRecord record, Integer colNum) {
				...
				if (fieldName.equals("targetName")) {
					final FolderSelector folderSelector = new FolderSelector("target", false);
	...
	boolean validate() {
		...
		for (ListGridRecord record : records) {
			...
			target.setId(Long.parseLong(record.getAttribute("targetId")));
