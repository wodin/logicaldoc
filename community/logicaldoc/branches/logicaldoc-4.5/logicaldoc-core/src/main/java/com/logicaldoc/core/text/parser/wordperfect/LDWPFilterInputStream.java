...
/**
 * a modified version of WPFilterInputStream to handle better accented
 * characters (tested with German documents), unfortunately it can introduces
 * some inconsistencies that may hamper the proper extraction of text on
 * documents of some particular WPD versions.
 * 
 * @author Alessandro Gasparini
 */
public class LDWPFilterInputStream extends FilterInputStream {
	...
	public LDWPFilterInputStream(InputStream in) {
	...
	public int read() throws IOException {
		int result = in.read();
		// pw.println("res: " + result + ", " +(char) result +", " +
		// Integer.toHexString(result));

		if (result >= 0x21 && result <= 0x7E) {
			...
			result = read();
		} else if (isInSpecialTable(result)) {
			// System.out.println("IS in Special Char");
			return substituteAccentedChars(result);
		} else if (result == 0xC0) {
	...
	private boolean isInSpecialTable(int result) {

		int[] intarry = new int[] { 0x05, 0x06, 0x1a, 0x1b, 0x20, 0x1c, 0x1d };
		return ArrayUtils.contains(intarry, result);
	}

	private int substituteAccentedChars(int result) {
	...
	public void closeExtraOut() {
		...
		try {
			...
			xxx = xxx.replaceAll("\\p{Cntrl} && ^\\n", " ");

			File file = new File("c:/tmp/wordperfect/myout.txt");
			FileUtils.writeByteArrayToFile(file, xxx.getBytes());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
