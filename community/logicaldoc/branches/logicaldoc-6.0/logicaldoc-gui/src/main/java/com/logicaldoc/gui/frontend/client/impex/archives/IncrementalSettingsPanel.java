...
public class IncrementalSettingsPanel extends VLayout {
	...
	public IncrementalSettingsPanel(GUIIncrementalArchive incremental,
			ChangedHandler changedHandler, FolderChangeListener folderListener) {
		this.incremental = incremental;

		form.setValuesManager(vm);
		form.setTitleOrientation(TitleOrientation.TOP);

		TextItem prefix = ItemFactory.newSimpleTextItem("prefix", "prefix",
				incremental.getPrefix());
		...
		IntegerItem frequency = ItemFactory.newIntegerItem("frequency",
				"frequency", incremental.getFrequency());
	...
	public boolean validate() {
		...
		if (!vm.hasErrors()) {
			...
			if (vm.getValueAsString("archivetype") != null)
				incremental.setType(Integer.parseInt(vm
						.getValueAsString("archivetype")));
			incremental.setFrequency(Integer.parseInt(vm
					.getValueAsString("frequency")));
			incremental.setFolder(folderSelector.getFolder());

			if (vm.getValues().get("template") != null) {
				String templateIdString = vm.getValues().get("template")
				...
				if (!templateIdString.isEmpty()) {
					...
					for (String selectionId : selection) {
						...
						currentTemplate
								.setId(Long.parseLong(selectionId.trim()));
						templates.add(currentTemplate);
					}
					incremental.setTemplates(templates
							.toArray(new GUITemplate[0]));
				}
			}
