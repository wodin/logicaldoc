...
/**
 * Uninstaller custom action for handling registry entries. The needed
 * configuration data are written at installation time from the corresponding
 * installer custom action. An external definiton is not needed.
 * 
 * @author Klaus Bartz
 */
	...
	public void beforeDelete(List<File> files) {
		...
		if (handler != null) {
			...
			try {
				...
				handler.deleteKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\LogicalDOC Community");
			} catch (Throwable e) {
				// throw new WrappedNativeLibException(e, messages);
			}
		}
